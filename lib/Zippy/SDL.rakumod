unit module Zippy::SDL;

### This is the original SDL2::Raw:ver<0.3>:auth<cpan:TIMOTIMO>
### adabted for Zippy
...
class SDL_Point is repr('CStruct') is rw is export( :DEFAULT :point ) {
    has int32 $.x;
    has int32 $.y;

		method add ( :$x = 0, :$y = 0 ) { $!x += $x.Int; $!y -= $x.Int }

		multi method reflect ( Bool :$over-x! ) { $!y = -$!y }
		multi method reflect ( Bool :$over-y! ) { $!x = -$!x }
		multi method reflect ( Bool :$origin! ) { $!x = -$!x; $!y = -$!y }
    # positional
    multi method new(Real $x, Real $y) { self.bless(x => $x.Int, y => $y.Int) }
...
multi infix:<*> (SDL_Point \a, SDL_Point \b) is export( :point ) {
  SDL_Point.new: :x( a.x * b.x ) :y( a.y * b.y )
}

multi infix:<*> ( Real \r, SDL_Point \b) is export( :point ) {
  SDL_Point.new: :x( r * b.x ) :y( r * b.y )
}


class SDL_Rect is repr('CStruct') is rw is export( :DEFAULT :rect ) {
    has int32 $.x;
    has int32 $.y;
    has int32 $.w;
    has int32 $.h;

		method has-intersection ( SDL_Rect $rect ) { SDL_HasIntersection( self, $rect ) }

		#method intersect-rect ( SDL_Rect $rect  ) {
		#	my SDL_Rect $result;
	  # SDL_IntersectRect( self, $rect, $result );
		#	$result;
		#}

    multi method shift( SDL_Point $xy ) { $!x += $xy.x; $!y += $xy.y }
    multi method shift(int $x, int $y) { $!x += $x; $!y += $y }
    multi method shifted($x, $y) { self.bless(x => $!x + $x, y => $!y + $y, :$!w, :$!h) }

    multi method resize( SDL_Point $xy ) { $!w += $xy.x; $!h += $xy.y }

    multi method center( --> SDL_Point ) { SDL_Point.new: $!x + ( $!w / 2 ), $!y + ( $!h / 2 ) }
    multi method center( Bool :$x! ) { $!x + ( $!w / 2 ) }
    multi method center( Bool :$y! ) { $!y + ( $!h / 2 ) }

    method clone() { self.bless(:$!x :$!y :$!w :$!h) }
...
sub SDL_HasIntersection( SDL_Rect $rect1, SDL_Rect $rect2) returns Bool is native($lib) is export {*}
sub SDL_IntersectRect(   SDL_Rect $rect1, SDL_Rect $rect2, SDL_Rect $result ) returns Bool is native($lib) is export {*}
...
sub SDL_Delay (uint32 $ms) is native($lib) is export {*}
...
enum IMG_INIT is export (

	JPG  => 0x00000001; # JPG
	PNG  => 0x00000002; # PNG
	TIF  => 0x00000004; # TIF
	WEBP => 0x00000008; # WebP

);

sub IMG_Init        ( int32        $flag                ) returns int32       is native('SDL2_image') is export {*}
sub IMG_Load        ( Str          $path                ) returns SDL_Surface is native('SDL2_image') is export {*}
sub IMG_LoadTexture ( SDL_Renderer $renderer, Str $path ) returns SDL_Texture is native('SDL2_image') is export {*}
